...
Una inspiración profunda puede incrementar la magnitud de la respuesta galvánica cutánea debido a que este tipo de respiración genera una respuesta del sistema nervioso autónomo, particularmente del sistema nervioso simpático. La actividad electrodérmica está estrechamente relacionada con la actividad de las glándulas sudoríparas ecrinas, las cuales reciben principalmente inervación simpática. Cuando aumenta esta actividad, se produce una mayor cantidad de sudor dentro de los conductos de estas glándulas, aumentando la conductividad eléctrica de la piel y disminuyendo su resistencia [2][5].  

La inspiración profunda actúa como un estímulo capaz de generar una respuesta transitoria de conductancia cutánea o SCR. Se ha observado experimentalmente que una inspiración profunda voluntaria puede producir una respuesta de conductancia detectable en las manos y los pies, por lo que incluso se utiliza como una prueba sencilla para comprobar el funcionamiento de sistemas de adquisición de actividad electrodérmica [5]. En estudios realizados con sujetos sanos se ha encontrado además que esta respuesta aparece poco después de iniciar la inspiración y está asociada con mecanismos centrales de activación simpática [6].  

En el caso del dispositivo desarrollado durante la práctica, este fenómeno se observa como una modificación del voltaje medido por la ESP32. Al aumentar la conductancia de la piel, cambia su resistencia eléctrica y, por lo tanto, también cambia el comportamiento del divisor utilizado en el circuito. Esto explica por qué durante la inspiración profunda puede observarse un aumento considerable de la GSR seguido de un regreso más lento hacia el valor inicial, comportamiento que también se describe en la guía del laboratorio [1].  

Pregunta 2: ¿Cuáles serían las ventajas y desventajas de utilizar la GSR como indicador de estrés?  

Una de las principales ventajas de la GSR es que permite obtener información relacionada con la activación del sistema nervioso simpático de manera relativamente sencilla, continua y no invasiva. La medición únicamente requiere electrodos ubicados sobre la piel y un circuito de adquisición, por lo que puede implementarse mediante dispositivos pequeños y portátiles como el sistema vestible desarrollado durante la práctica [2][7]. Además, la GSR responde rápidamente ante cambios de activación fisiológica y permite analizar tanto una componente basal o tónica como respuestas transitorias asociadas con determinados estímulos [2].  

Otra ventaja es que puede realizarse un monitoreo continuo mientras la persona desarrolla diferentes actividades. Esto permite utilizar dispositivos vestibles para estudiar situaciones relacionadas con concentración, carga mental o estrés sin depender necesariamente de equipos de laboratorio de gran tamaño. Diferentes estudios han mostrado que la actividad electrodérmica puede ser útil dentro de sistemas destinados a reconocer estrés y que su integración en dispositivos vestibles tiene aplicaciones potenciales para el monitoreo en condiciones reales [7].  

Sin embargo, una de sus principales desventajas es que la GSR no es una medida específica del estrés. Lo que realmente refleja es una modificación en la actividad electrodérmica asociada con la activación simpática. Por esta razón, una respuesta elevada también puede aparecer por emociones, dolor, respiración profunda, atención, actividad física u otros estímulos. En consecuencia, un aumento de la GSR no permite afirmar por sí solo que una persona se encuentra estresada [2][3][5].  

También existe una alta variabilidad entre personas. Los niveles basales de conductancia pueden cambiar dependiendo de las características de la piel, cantidad de sudor, posición de los electrodos y condiciones particulares del sujeto. Esto significa que un mismo valor de GSR no necesariamente representa el mismo nivel de activación en dos individuos diferentes, por lo que resulta conveniente realizar una calibración previa y trabajar con cambios respecto al valor basal de cada persona [7][8].  

Otra limitación importante corresponde a los artefactos de movimiento. Al caminar, escribir, mover la mano o modificar la presión entre el electrodo y la piel pueden producirse cambios en la señal que no corresponden a una verdadera respuesta fisiológica. También pueden influir factores ambientales como la temperatura y la humedad, así como el estado de los electrodos y la calidad del contacto con la piel [8]. Esto es especialmente importante en un dispositivo vestible como el construido en la práctica, debido a que está diseñado para utilizarse mientras el sujeto realiza distintas actividades.  

Por lo tanto, la GSR representa una alternativa útil para estimar cambios en la activación fisiológica relacionados con el estrés, principalmente por su bajo costo, facilidad de adquisición y posibilidad de monitoreo continuo. Sin embargo, para obtener una estimación más confiable del estrés sería conveniente combinarla con otras variables fisiológicas, como frecuencia cardíaca, variabilidad de la frecuencia cardíaca, temperatura cutánea o respiración, además de considerar el contexto en el cual se realizó la medición.  
